		...
		if (fs.existsSync(i18nFolderPath)) {
			if (platform === 'android') {
				fs.readdirSync(i18nFolderPath).forEach(function(lang) {
					if (lang === 'en') {
						var outputFile = path.join(appResourcesDirectoryPath, 'Android', 'values', 'strings.xml');
					} else {
						var outputFile = path.join(appResourcesDirectoryPath, 'Android', 'values-' + lang, 'strings.xml');
					}

					fs.copy(path.join(i18nFolderPath, lang, 'strings.xml'), outputFile,{clobber:true}, function(err) {
						if (err) {
							console.error(err);
							reject();
						}
